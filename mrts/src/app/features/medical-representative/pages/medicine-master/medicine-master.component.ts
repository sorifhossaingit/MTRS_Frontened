...
addToCart(product: InventoryProduct): void {
  ...
  const existing = this.cart.find(
    x => x.inventoryId === product.inventoryId
  );
...
validateCartQty(item: CartItem): void {
  ...
  if (!item.orderQty || item.orderQty < 1) {
    item.orderQty = 1;
  }
...
getSubTotal(): number {
  ...
  return this.cart.reduce(
    (sum, item) => sum + (item.orderQty * item.sellingPrice),
    0
  );
...
getSelectedStockistName(): string {
  ...
  return this.cart[0].stockistName;
...
getTotalAmount(): number {
  ...
  return this.cart.reduce((sum, item) => {
    ...
    return sum + (item.orderQty * item.sellingPrice);
